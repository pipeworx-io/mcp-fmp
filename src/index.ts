...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Financial Modeling Prep');
}
...
/** A screener sent no criteria is not a screen — it returns an arbitrary slice
 *  of the whole exchange, which reads as "here are the matching stocks". */
const SCREENER_SCHEMA = { type: 'object' as const, properties: {
  marketCapMoreThan: n('Minimum market cap in USD, e.g. 1000000000 for $1B+.'),
  marketCapLowerThan: n('Maximum market cap in USD.'),
  priceMoreThan: n('Minimum share price.'), priceLowerThan: n('Maximum share price.'),
  betaMoreThan: n('Minimum beta.'), betaLowerThan: n('Maximum beta.'),
  volumeMoreThan: n('Minimum average volume.'), volumeLowerThan: n('Maximum average volume.'),
  dividendMoreThan: n('Minimum dividend yield.'), dividendLowerThan: n('Maximum dividend yield.'),
  sector: s('e.g. "Technology", "Healthcare", "Energy", "Financial Services".'),
  industry: s('e.g. "Semiconductors", "Biotechnology".'),
  country: s('2-letter country code, e.g. "US".'),
  exchange: s('e.g. "NASDAQ", "NYSE".'),
  isEtf: s('"true" | "false"'), isActivelyTrading: s('"true" | "false"'),
  limit: n('Max rows (default 100). At least one filter should be set, or this is a slice of the whole exchange rather than a screen.'),
} };
...
const tools: McpToolExport['tools'] = [
  ...
  { name: 'stock_screener', description: 'Stock screener (paid).', inputSchema: SCREENER_SCHEMA },
  ...
  { name: 'earnings_calendar', description: 'Upcoming and recent earnings dates with EPS and revenue estimates. Pass `symbol` for one company; omit it for the market-wide calendar. FMP serves this endpoint market-wide only, so a symbol is filtered on our side and the response says how wide a window was searched.', inputSchema: { type: 'object', properties: { symbol: { type: 'string', description: 'Ticker to filter to, e.g. "CRM". Omit for the whole market.' }, from: { type: 'string' }, to: { type: 'string' } } } },
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (!apiKey) throw new Error('FMP requires an API key: pass your key as the _apiKey argument (free at https://site.financialmodelingprep.com/developer/docs).');
  const get = async (path: string, params?: Record<string, unknown>) => {
    ...
    // A 402 here is an access wall, not a throttle, and the difference decides
    // whether anybody ever sees it (fleet #573). Booked as `upstream_throttled`
    // this was ~490 failed calls a week — fmp_quote 282, fmp_profile 108,
    // quote_short 101 over seven days — sitting in the one class the fleet's
    // problem-tools triage deliberately excludes, on the reasonable assumption
    // that a throttle is somebody's rate limit rather than our problem. A paid-
    // plan wall is permanent for a free-tier caller; it never clears by waiting.
    //
    // It classifies as `auth_required` now, and NOT via a routing token: the
    // classifier's auth rule already matches `_apiKey`, so naming the BYO route
    // in the sentence a caller reads is what files it correctly. That is the
    // right class on the merits too — auth_required means "an expected access
    // wall, a credential we do not hold", which is exactly this. It is also
    // actionable, which `upstream_throttled` was not: a caller with their own
    // paid FMP key can run this endpoint today.
    //
    // Deliberately NOT `payment_required:`. In this codebase that class means
    // OUR paywall — the classifier produces it for "out of credits / add
    // credits / free daily allowance", x402.ts emits it as the payment-rail
    // code, and registry-api excludes it from the broken-tool count on purpose.
    // It is the caller owing Pipeworx, not Pipeworx owing a vendor. Reusing the
    // word for a vendor's tier would file a dead endpoint as a healthy billing
    // state and make it invisible a second way.
    if (res.status === 402) {
      throw new Error(
        `FMP: this endpoint is not on the plan behind the key being used. Pipeworx's shared FMP key is a free-tier one, and FMP moved much of what used to be free behind Starter/Premium in Aug 2025 — so ${path} answers 402 no matter how long you wait. Pass your own paid FMP key as _apiKey and this call works immediately; plans at https://site.financialmodelingprep.com/pricing-plans. For quotes and profiles without a key, yahoo-finance and stooq cover the same ground.`,
      );
    }
    // A real throttle, and the only thing here that IS one. Tokened explicitly
    // so it cannot drift into the same misfiling the 402 above just came out of.
    if (res.status === 429) throw new Error('upstream_throttled: FMP rate limit reached (the free tier allows 250 calls/day). This one does clear — retry later, or pass your own _apiKey for a higher quota.');
    // The same wall in a third disguise, and this one was landing in `error` —
    // the class that means PIPEWORX HAS A DEFECT, which put a healthy pack on
    // the problem-tools list for a bug it does not have.
    //
    // FMP answers some plan-gated endpoints with `404 []` instead of a 402.
    // Determined rather than assumed: insider_trading returns 404 [] for TSLA,
    // NVDA and META alike, and all three plainly have insider activity — so it
    // is not "no rows for this symbol", it is the endpoint declining to answer.
    // The three tools this pack itself labels "(paid)" are the ones affected.
    //
    // The message names BOTH readings, because a 404 with an empty body is
    // genuinely ambiguous and I would rather say so than assert the wrong one:
    // an empty result and a closed endpoint look identical from here. Naming
    // `_apiKey` files it as auth_required either way, which beats `error` under
    // both readings.
    if (res.status === 404) {
      const body = (await res.text().catch(() => '')).trim();
      if (!body || body === '[]' || body === '{}') {
        throw new Error(
          `FMP returned 404 with an empty body for ${path}. That means either this symbol has no rows, or — for the endpoints FMP gates by plan, which is where this mostly happens — the plan behind the key being used does not include it. Pipeworx's shared FMP key is free-tier. Pass your own paid FMP key as _apiKey to tell the two apart; plans at https://site.financialmodelingprep.com/pricing-plans.`,
        );
      }
      throw new Error(`not_found: FMP returned 404 for ${path} — ${body.slice(0, 140)}`);
    }
    if (!res.ok) throw await httpError(res, 'FMP');
    return res.json();
  };
  ...
  switch (name) {
    ...
    case 'earnings_calendar': {
      // FMP's earnings-calendar is MARKET-WIDE and ignores an unknown `symbol`
      // query param. It used to be forwarded and silently dropped, so asking
      // about CRM returned ~50 unrelated companies and no error — which an
      // agent summarises as "Salesforce has no upcoming earnings" (fleet #260).
      // The endpoint has no per-symbol form, so filter here and be explicit
      // about the window that was actually searched.
      const wanted = typeof args.symbol === 'string' ? args.symbol.trim().toUpperCase() : '';
      const { symbol: _drop, ...rest } = args;
      const rows = (await get('/earnings-calendar', rest)) as Array<Record<string, unknown>>;
      if (!wanted) return rows;
      const list = Array.isArray(rows) ? rows : [];
      const hits = list.filter((r) => String(r.symbol ?? '').toUpperCase() === wanted);
      if (!hits.length) {
        return {
          found: false,
          reason: 'no_scheduled_earnings',
          symbol: wanted,
          window: { from: args.from ?? null, to: args.to ?? null },
          searched_rows: list.length,
          hint: `No earnings date for ${wanted} among the ${list.length} rows FMP returned for this window. FMP serves this calendar market-wide and truncates, so widen from/to — and note this is an absence of a scheduled date in the searched window, not a statement that the company has no earnings.`,
        };
      }
      return { symbol: wanted, count: hits.length, searched_rows: list.length, earnings: hits };
    }
